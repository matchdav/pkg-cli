...
module.exports = class Command extends BaseCommand {
    async info(pkg, ...fields) {
        const { stdout } = await exec(`npm view ${pkg} ${fields.join(' ')} -j --registry $(npm config get @ingrooves:registry)`);
    ...
    async run(filters) {
        ...
        const results = await Promise.all(Object.keys(this.pkg.data.dependencies).
